// Package game implements the Asteroids game loop, rendering, input, and audio.
package game
...
// New constructs a game at the title screen with fresh session state.
func New() *Game {
	g := &Game{
...
// Close releases audio players owned by the game.
func (g *Game) Close() {
	if g.Sound != nil {
		g.Sound.Close()
	}
